...
dst_dir_neun = '/dcs04/lieber/marmaypag/spatialDLPFC_SCZ_LIBD4100/processed-data/RealPNN/single_channels_segmented/NeuN/slide16/'
...
for img_path in os.listdir(img_dir):
    if img_path.endswith(".tif") and ('V13F27-336') in img_path:
        ...
        # cv2.imwrite(dst_dir_neun + img_path.split('.')[0] + '_neun_.tif', neun)
        neun_c = cv2.cvtColor(neun,cv2.COLOR_BGR2RGB)
        gray = cv2.cvtColor(neun_c,cv2.COLOR_RGB2GRAY)
        _,thresh = cv2.threshold(gray,100 , 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) #_INV;;; np.mean(gray)
