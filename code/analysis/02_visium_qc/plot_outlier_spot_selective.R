...
## Fetch demo info
raw_spe$dx <- metadata(raw_spe)$dx_df$dx[
  match(
    raw_spe$sample_id,
    metadata(raw_spe)$dx_df$sample_id
  )
]

raw_spe$brnum <- metadata(raw_spe)$dx_df$subject[
  match(
    raw_spe$sample_id,
    metadata(raw_spe)$dx_df$sample_id
  )
...
# merge in outlier info to SPE
raw_spe$all_outlier <- tot_outlier_df[raw_spe$key, "all_outlier"]
raw_spe$discard <- tot_outlier_df[raw_spe$key, "remove"]

# keep in_tissue spots only
spe <- raw_spe[, raw_spe$in_tissue == TRUE]
...
spot_plot_list <- spe$sample_label |>
  ...
  map(.f = function(.smp) {
    ...
    ret_plot <- sub_spe |>
      make_escheR() |>
      add_fill(var = "discard", point_size = 0.8)
