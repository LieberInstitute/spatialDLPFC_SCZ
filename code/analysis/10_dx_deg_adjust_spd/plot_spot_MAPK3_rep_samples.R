...
# Make Spot Plot ----
## Function to create escheR spot plot ----
quick_plot_gene_exp <- function(spe, gene_symbol) {
  # Examine if the gene is in the row data
  stopifnot(
    "Gene Symbol not found in the data set" =
      gene_symbol %in% rowData(spe)$gene_name
  )

  # Acquire it's Ensembl ID
  gene_ensembl <- rowData(spe)$gene_id[
    rowData(spe)$gene_name == gene_symbol
  ]

  # browser()
  var_name <- paste0("log_", gene_symbol)
  spe[[var_name]] <- logcounts(spe)[gene_ensembl, ]
  ...
  plot_list <- plot_list |> lapply(FUN = function(.p) {
    .p +
      scale_fill_gradient(
        limits = c(0, max(spe[[var_name]], na.rm = TRUE)),
...
quick_plot_gene_exp(spe, gene_symbol = "MAPK3")
quick_plot_gene_exp(spe, gene_symbol = "ATP2B4")
quick_plot_gene_exp(spe, gene_symbol = "KCNK1")
quick_plot_gene_exp(spe, gene_symbol = "SNAP25")
quick_plot_gene_exp(spe, gene_symbol = "SST")

quick_plot_gene_exp(spe, gene_symbol = "SERPINA3")




# spe$log_MAPK3 <- logcounts(spe)["ENSG00000102882", ] # MAPK3
...
#   .p +
#     scale_fill_gradient(
#       limits = c(0, max(spe$log_MAPK3, na.rm = TRUE)),
#       low = "white", high = "black"
#       # guide = guide_legend(override.aes = list(size = 7))
#     ) +
#     scale_color_manual(
