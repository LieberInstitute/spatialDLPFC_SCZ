...
dx_mod <- model.matrix(
  ~ 0 + norm_PRS + fnl_spd + age + sex + slide_id + genotype_PC1 + genotype_PC2,
  colData(sce_pseudo)
)
...
# Descriptive Stats ----
sum(fnl_res$adj.P.Val < 0.1)

hist(fnl_res$P.Value, breaks = 100,
     main = "P-value Distribution for 2PC+normalized PRS DEG test",
     xlab = "P-value")
...
write_csv(fnl_res, file = here(
  ...
  "2PC_norm_PRS_DEG_test_res_PRECAST07_donor_spd.csv"
))
