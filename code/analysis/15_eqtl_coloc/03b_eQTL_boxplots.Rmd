...
plot_eQTL_boxplots <- function(plotdata, eqtl_pairs, use_raw = FALSE, restrict_donors = NULL, title = NULL) {
  ...
  # Build orientation metadata for current variants against pvar allele order
  ...
  plot_list <- lapply(seq_len(nrow(eqtl_pairs)), function(i) {
    ...
    # Convert dosage to reference-genome orientation when pvar is swapped.
    # This drives BOTH x-axis genotype grouping and labels (id_REF/id_ALT coding).
    geno_dosage_ref <- if (!is.na(ori_sign) && ori_sign == -1L) 2 - geno_dosage_raw else geno_dosage_raw
    geno_labels <- make_geno_labels(geno_dosage_ref, id_ref, id_alt)

    # Order genotypes by dosage within this variant
    geno_order <- unique(geno_labels[order(geno_dosage_ref)])
    ...
    # Reorient effect direction to variant_id REF/ALT convention:
    # slope_ref = slope * orientation_sign (sign flips when swapped).
    # p-values are unchanged by orientation.
    beta_raw <- if ("slope" %in% names(eqtl_pairs)) eqtl_pairs$slope[i] else NA
    beta_val <- orient_effect(beta_raw, ori_sign)
