...
gwas_vars <- fread(file.path(refdir, "scz_gwas_wide.scz.tqtl-matched.tab.gz"))
...
# test prep function -- assumes spe, covar_file were previously loaded
plotdata <- prep_eQTLPlotData(spe, geno, covar_file)
```

```{r test/validate prep_eQTLPlotData}
...
# ---- Test with neun data ---- assumes indr already loaded for the currently prepared spe data
# Find 3 eQTLs with high significance and large effect size
#indr_neun <- indres[["neun"]]
indr[, abs_slope := abs(slope)]
# Filter for high significance and effect, ensure 3 genotype classes visible (reasonable MAF)
top_eqtls <- indr[pval_nominal < 1e-8 & abs_slope > 1.0 & af > 0.15 & af < 0.85][order(-abs_slope)][c(2,3)]
...
p <- plot_eQTL_boxplots(plotdata, top_eqtls) #, title = sprintf("%s eQTL examop", dsname))
p
```
