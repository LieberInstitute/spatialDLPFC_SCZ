...
```{r check for MAPK3 in vasc, pnn and neun}
...
eqtls <- indr[gene_name=="MAPK3"]
stopifnot(nrow(eqtls)>0)
#top_eqtls[, .(phenotype_id, variant_id, slope, pval_nominal, pval_perm, af)]

# Create plot
## this might have to be customized further
p <- plot_eQTL_boxplots(plotdata, eqtls) #, title = sprintf("%s eQTL examop", dsname))
print(p)
