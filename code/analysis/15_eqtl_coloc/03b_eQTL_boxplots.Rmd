...
variant_id (genomic) REF/ALT vs GWAS A1:
...
A1 == pvar ALT (100%) => because GWAS data was harmonized with pvar REF/ALT orientation
flipping the beta sign accordingly.
(see 03_eqtl_explore.Rmd for details on GWAS-tensorQTL matching/harmonization)

A1 is the ALT allele in 75% of cases
...
plot_eQTL_boxplots <- function(plotdata, eqtl_pairs, use_raw = FALSE, restrict_donors = NULL, title = "auto", gwas_eff = FALSE, ds = NULL) {
  ...
  title_by_gene <- title_pairs[, .(variant_list = paste(unique(variant_label), collapse = ", ")), by = gene_name]
  ...
  p <- ggplot(plot_dt, aes(x = genotype, y = expression)) +
    ...
    scale_y_continuous(expand = expansion(mult = c(0.09, 0.05))) +
    labs(
      x = x_axis_label,
...
#print(plot_eQTL_boxplots(plotdata, ieqtls, ds=ds, gwas_eff = TRUE))
```
