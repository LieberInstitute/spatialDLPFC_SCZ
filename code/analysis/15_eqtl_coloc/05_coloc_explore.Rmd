...
fgtgwas <- file.path(refdir, 'GWAS_SCZ_tqtl-matched.tab.gz') ## this was created in 03_eqtl_explore.Rmd
...
```{r SKIP example stats per domain}
fc <- cdts[["spd01"]]
sum_stats <- fc[, .(
  N=.N,
...
```{r aggregate data and save table}
fc_all <- rbindlist(cdts, idcol="ds")
fc_all[, gene_name:=g2sym[fid]]
setcolorder(fc_all, c("ds", "fid", "gene_name", "lead_snp", "cls", "nsnps"))
## add rsid for lead SNPs where available, for easier reporting and follow-up lookup
fc_all[gwas_vars, rsid := i.rsid, on = .(lead_snp = variant_id)]


fcoloc <- file.path(datadir, "coloc", "coloc_abf_results.tsv.gz")

if (!file.exists(fcoloc)) {
   fwrite(fc_all, file=fcoloc, sep="\t")
}
...
Not in the coloc results, other SNPs for MAPK3 have low PP4, high PP34 (likely distinct causal variants).

> fc_all[gene_name=="MAPK3"]
         ds             fid gene_name           lead_snp                    cls nsnps kept_by          PP0          PP1        PP2       PP3        PP4      PP34
     <char>          <char>    <char>             <char>                 <char> <num>  <char>        <num>        <num>      <num>     <num>      <num>     <num>
1:     neun ENSG00000102882     MAPK3 chr16:29983601:C:T likely_distinct_causal   733    PP34 1.345527e-10 2.506781e-09 0.04776160 0.8897589 0.06247953 0.9522384
2:     vasc ENSG00000102882     MAPK3 chr16:29943634:A:G likely_distinct_causal   732    PP34 4.983703e-10 1.197340e-09 0.17690437 0.4246164 0.39847925 0.8230956
3: neuropil ENSG00000102882     MAPK3 chr16:29983601:C:T likely_distinct_causal   733    PP34 4.200967e-11 2.685790e-09 0.01491199 0.9533316 0.03175638 0.9850880
   PP4_over_PP34 lead_snp_PPH4 lead_snp_PPsho cs95_n_snp    p1    p2   p12             cat
           <num>         <num>          <num>      <int> <num> <num> <num>          <char>
1:    0.06561333    0.07354177    0.004594855         45 1e-04 1e-04 1e-05 distinct_causal
2:    0.48412267    0.30604491    0.121952546         45 1e-04 1e-04 1e-05 distinct_causal
3:    0.03223710    0.13209777    0.004194948         44 1e-04 1e-04 1e-05 distinct_causal
...
pdt <- pdt[all_grid, on = .(ds, cat)]
pdt[is.na(N), `:=`(N = 0L, genes = NA_character_)]
...
```{r stacked bars plot}
pdt[, ds := as.character(ds)]
ds_levels <- names(ds2name)
ds_labels <- unname(ds2name)
p <- ggplot(
  pdt,
  aes(
    x = factor(ds, levels = ds_levels, labels = ds_labels),
    y = N,
    fill = cat
  )
) +
  ...
  geom_text(
    data = tot,
    aes(x = factor(ds, levels = ds_levels, labels = ds_labels), y = Ntot, label = paste0("n=", Ntot)),
    inherit.aes = FALSE,
    hjust = -0.1,
    size = 3
  ) +
  ...
  labs(
    ...
    title = "Potential colocalization loci by dataset",
    subtitle = paste0(
      "strong_coloc: PP34 >= 0.8 & PP4/(PP34) >= 0.9; ",
      "distinct_causal: PP34 >= 0.8 & PP4/(PP34) < 0.5; ",
      "follow_up: PP34 >= 0.8 | PP4 >= 0.8"
    )
  ) +
  ...
  scale_fill_manual(
    ...
    breaks = c("strong_coloc", "follow_up")
  )
print(p)
```
