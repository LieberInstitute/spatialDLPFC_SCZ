...
which_neighbors <- function(var, values) {
  # TODO: edit this part values is just boeleans
    ...
    # This doesn't matter that much in our data
    # res[!res %in% has_path]
}

neighbor_pnn_pos <- which_neighbors("pnn_pos", TRUE)
neighbor_neuropil_pos <- which_neighbors("neuropil_pos", TRUE)
neighbor_neun_pos <- which_neighbors("neun_pos", TRUE)
neighbor_vasc_pos <- which_neighbors("vasc_pos", TRUE)
