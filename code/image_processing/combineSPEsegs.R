...
p21 = ggplot(coldata_df, aes(x = dx, y = PWFA)) + geom_boxplot(outlier.shape = NA) +
		labs(title = "Distribution of WFA by Dx",x = "Dx",y = "PWFA >= 25%") + geom_jitter(width = 0.2, alpha = 0.5) +
...
p32 = ggplot(coldata_df, aes(x = sex, y = PWFA, fill = dx)) + geom_violin(trim = FALSE, alpha = 0.7) +
		labs(title = "Distribution of WFA by sex",x = "sex",y = "PWFA >= 25%") + 
				theme_minimal()+ylim(0.25,1) 

			    pdf(here("plots","image_processing","pnn_EDA_PWFA.pdf"), width = 10, height = 8)
			    print(p1)
				print(p11)
				print(p12)
				print(p2)
...
p32 = ggplot(filtered_data, aes(x = sex, y = IWFA, fill = dx)) + geom_violin(trim = FALSE, alpha = 0.7) +
		labs(title = "Distribution of WFA by sex",x = "sex",y = "IWFA >0") + 
			    ...
			    pdf(here("plots","image_processing","pnn_EDA_IWFA.pdf"), width = 10, height = 8)
			    print(p1)
				print(p11)
				print(p12)
				print(p2)
...
p1 = ggplot(proportion_data, aes(x = PRECAST_10, y = Proportion, fill = CNWFA_GT_0)) +
...
proportion_data <- coldata_df_ntc %>%
  group_by(sample_id, PRECAST_10, PVALB = gene > 1) %>%
  summarise(Count = n()) %>%
  ungroup() %>%
  group_by(sample_id, PRECAST_10) %>%
  mutate(Proportion = Count / sum(Count))

p2 = ggplot(proportion_data, aes(x = PRECAST_10, y = Proportion, fill = PVALB)) +
          geom_bar(stat = "identity", position = "stack") +
          labs(title = "Proportion of PVALB+ by spD in NTCs", x = "Sample ID", y = "Proportion", fill = "PVALB >   1") +
          theme_minimal()+facet_wrap(~ sample_id)+
          theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

		  pdf(here("plots","image_processing","pnnVSpvalb_ntc.pdf"), width = 10, height = 8)
		  print(p1)
		  print(p2)
		  dev.off()	  	  
...
proportion_data <- coldata_df_scz %>%
  ...
  p2 = ggplot(proportion_data, aes(x = PRECAST_10, y = Proportion, fill = PVALB)) +
          ...
          theme_minimal()+facet_wrap(~ sample_id)+
				  ...
				  pdf(here("plots","image_processing","pnnVSpvalb_scz.pdf"), width = 10, height = 8)
				  print(p1)
				  print(p2)
				  dev.off()	  	  
